...
- **根因**：`game_state._analyze` 翻后重放里「金额未知的全下」(协议 -2) 只夹到
  「本轮最大注」→ `to_call` = 占位 1 → bot 以为只要跟 1 个筹码 → 任何牌都跟 →
  `_normalize`(any_allin) 变全押。**翻前分支一直是保守口径**（未知 → 按需全押），
  翻后漏了 → 已对齐：`rb[opp]=max(rb)+my_chips` 且 `_opp_round_known=False`。
- **教训**：盘上行为与代码不符时，先穷举输入形态（金额缺失 / my_id 反转 / 字段类型），
  再怀疑规则；日志里的「规则1/4 大注弃牌」只是 `_winning_rule()` 的兜底标签（无候选
  匹配时的默认名），不是真规则。
- **翻后 all-in 门槛速查**（面对对手 all-in 时的判定顺序）：
  规则2 锁赢/防锁赢 → 河牌裸公对陷阱 → 公面同花威胁 → 突袭大注(赌注 eq 打折 0.15、
  牌型<三条直接弃) → `thr = eff_req + margin ± 档位 + 规则17分档 − 规则16放宽`。
